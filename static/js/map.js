(function(){
    ...
    const st = { map:null, marker:null, track:[], poly:null, last:null, follow:true, drawing:true, wps:[], wpLayer:null, minDist:0.00001, centered:false };
    ...
    const ui = {lat:g('lat'),lon:g('lon'),alt:g('alt'),time:g('time'),speed:g('speed'),heading:g('heading'),mode:g('statusMode'),fix:g('statusFix'),sat:g('statusSat'),hdop:g('statusHdop'),
                            c:g('btnCenter'),t:g('btnToggleTrack'),clr:g('btnClearTrack'),wpName:g('wpName'),add:g('btnAddWp'),exp:g('btnExportWp'),wclr:g('btnClearWp'),list:g('waypointList')};
    ...
    async function poll(){ const dRaw=await j(API.position); let d=dRaw; // Normalizacja pól
        ...
        if(d && !d.error && typeof d.lat==='number' && typeof d.lon==='number'){
            ...
            if(st.drawing && need(d.lat,d.lon)) addPoint(d.lat,d.lon);
